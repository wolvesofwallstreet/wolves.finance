...
class Store {
  ...
  _doStakeAdd = async (payloadContent: PayloadContent) => {
    ...
    try {
      ...
      const tx:
        ...
        | undefined = await stakeContract?.stake(stakeAmount);
      emitter.emit(STAKE_ADD, {
        status: 'tx',
        tx: tx?.hash,
      } as StatusResult);
  ...
  _doStakeClaim = async (payloadContent: PayloadContent) => {
    try {
      ...
      const tx:
        | ethers.ContractTransaction
        | undefined = await stakeContract.getReward();
  ...
  _doStakeExit = async (payloadContent: PayloadContent) => {
    try {
      ...
      const tx:
        | ethers.ContractTransaction
        | undefined = await stakeContract.exit();
  ...
  _doSftBuy = async (payloadContent: PayloadContent) => {
    ...
    try {
      ...
      const tx:
        | ethers.ContractTransaction
        | undefined = await sftMintContract?.mintWowsSFT(
        this.address,
        id.toNumber() >> 8,
        id.toNumber() & 0xff,
        { gasLimit: 420000 }
  ...
  _doSftUnlock = async (payloadContent: PayloadContent) => {
    ...
    try {
      ...
      const tx:
        | ethers.ContractTransaction
        | undefined = await tradeFloorContract.burn(this.address, id, 1);
      emitter.emit(SFT_UNLOCK, {
        status: 'tx',
        tx: tx?.hash,
      } as StatusResult);
  ...
  _doCFolioItemBuy = async (payloadContent: PayloadContentCFolioItem) => {
    ...
    try {
      ...
      const tx: ethers.ContractTransaction = await sftMintContract?.mintCFolioItemSFT(
        this.address,
        cfolioType,
        sftTokenId,
  ...
  _doSftUpgrade = async (payloadContent: PayloadContent) => {
    try {
      ...
      const tx: ethers.ContractTransaction = await this.sftEvaluatorContract.setRewardRate(
        payloadContent.id,
        true
      );
